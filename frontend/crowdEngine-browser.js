(function () {
  ...
  const WARNING_COOLDOWN = 60000; // 60s — max 1 warning per zone per minute
  ...
  async function processZone(zone) {
    ...
    // Simulate new data — smooth drift (±10) instead of fully random
    const dDelta = Math.floor(Math.random() * 21) - 10;
    zone.density = Math.max(10, Math.min(80, zone.density + dDelta));
    const tDelta = Math.floor(Math.random() * 5) - 2;
    zone.temperature = Math.max(25, Math.min(38, zone.temperature + tDelta));
    ...
    if (newRiskLevel === 'HIGH' && zone.riskLevel !== 'HIGH') {
      ...
      const now = Date.now();
      if (!warningCooldowns[zone.zoneId] || (now - warningCooldowns[zone.zoneId]) > WARNING_COOLDOWN) {
        warningCooldowns[zone.zoneId] = now;
        warnings.push({
          warningId: 'CW-' + zone.zoneId + '-' + now,
          type: 'CROWD_SURGE', zoneId: zone.zoneId, label: zone.label,
          location: zone.location, floor: zone.floor, severity: 'MEDIUM',
          status: 'WARNING',
          description: `High crowd density at ${zone.label}. Density: ${zone.density}, Temp: ${zone.temperature}°C.`,
          timestamp: new Date().toISOString(), autoGenerated: true
        });
      }
